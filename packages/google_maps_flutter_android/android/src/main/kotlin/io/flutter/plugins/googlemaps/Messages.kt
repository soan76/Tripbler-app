...
interface MapsApi {
   ...
   * false = light mode
   */
  fun setMapColorScheme(isDarkMode: Boolean)
  /**
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: MapsApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.google_maps_flutter_android.MapsApi.setMapColorScheme$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val isDarkModeArg = args[0] as Boolean
            val wrapped: List<Any?> = try {
              api.setMapColorScheme(isDarkModeArg)
              listOf(null)
            } catch (exception: Throwable) {
              MessagesPigeonUtils.wrapError(exception)
            }
