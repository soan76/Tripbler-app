...
void SetUpFGMMapsApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FGMMapsApi> *api, NSString *messageChannelSuffix) {
  ...
  /// false = light mode
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.google_maps_flutter_ios.MapsApi.setMapColorScheme", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FGMGetGoogleMapsFlutterPigeonMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setMapColorScheme:error:)], @"FGMMapsApi api (%@) doesn't respond to @selector(setMapColorScheme:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        BOOL arg_isDarkMode = [GetNullableObjectAtIndex(args, 0) boolValue];
        FlutterError *error;
        [api setMapColorScheme:arg_isDarkMode error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
